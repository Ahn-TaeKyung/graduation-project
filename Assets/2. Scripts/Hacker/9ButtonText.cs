...
public class ButtonText9 : MonoBehaviour
{
    private NineButtons patternManager;
    public Transform panelTransform; // 버튼의 판(Panel) Transform (Inspector에서 할당)

    void Start()
    {
        if (patternManager == null)
            patternManager = GetComponent<NineButtons>();
        ...
        // 아래(혹은 원하는 위치)로 이동
        labelObj.transform.localPosition = new Vector3(0.15f, -0.35f, 0.51f); // Y값은 베이스 두께·크기에 맞게 조절
        ...
        textMeshPro.rectTransform.sizeDelta = new Vector2(0.7f, 1);
